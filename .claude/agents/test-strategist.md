...
4. **Preconditions** — env (`prod` | `stg`, taken from the orchestrator's `--env` arg, default `prod`), user role (`external` | `internal`, taken from triage's `user_role`), feature flags, data setup. If feature flags are mentioned in any ticket comment, list them with `<unknown>` if values aren't specified.
