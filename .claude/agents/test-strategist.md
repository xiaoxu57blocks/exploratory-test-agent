...
4b. **Apply testing-scope rules before writing scenarios.** Read `context/testing-scope/index.md`. Then:
   - If the PR diff contains **only** visual/layout changes (CSS values, spacing, color, icon size) with no business logic — this ticket should have been skipped in triage. If it wasn't, stop, flag it to the orchestrator, and do not write scenarios.
   - If the PR diff contains **both** business logic and visual tweaks, apply `context/testing-scope/ui-logic-mixed-scope.md`: write scenarios for the logic side only, exclude all style/spacing assertions, and add an explicit **Out of scope** note naming the visual changes and the lines in the diff where they appear.

4c. **Consult the feature flag context before scanning.** Read `context/feature-flags/index.md` first. For each flag listed there whose gated path appears in the PR diff (e.g. a changed file under `src/packages/app-v2/` → `feature-case-agent`), load the corresponding detail file (e.g. `context/feature-flags/app-v2.md`) and apply its rules. These context rules catch flags that are **never mentioned in the diff itself** — they exist at the routing/rendering layer above the changed code and will be missed by grep alone.

4d. **Run the three-pass gate scan** (see Feature flag detection above) on every PR diff you just read. Produce a gate inventory before writing any scenarios:
